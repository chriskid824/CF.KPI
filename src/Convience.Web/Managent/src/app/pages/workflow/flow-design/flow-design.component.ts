...
export class FlowDesignComponent implements OnInit {
  ...
  nodes = [
    { key: 'start', name: '開始节点', icon: 'play-circle' },
  ...
  addStartNode(id, x, y, title = "開始节点") {
  ...
  addNode(id, x, y, title, hasDblClickEvent = false) {
    ...
    this._renderer.listen(node, 'mousedown', event => {
      ...
      // 绑定四個元素作為border的目的是為了以後修改為调整大小的瞄点
  ...
  dropZone(event) {
    ...
    // 取得offsetx时，如果有子元素會冒泡，将子元素的offset設置
    ...
    switch (this._draggedKey) {
      case 'start':
        if (this._nodeDataList.find(d => d.nodeType == 0)) {
          this._messageService.error('已经有一個開始节点了！');
        } else {
